...
const _removeLocaleCode = (str: string | number | symbol) => {
    return str
        .toString()
        .replace(new RegExp(`_(${localeCodes.join('|')})$`), '')
}

export const getLocaleValue = <O extends object, K extends keyof O>(
    obj: O,
    key: K,
    locale: string,
) => {
    const currentCode =
        localeCodes.find((code) => code === locale) ?? localeCodes[0]
    const noLocaleKey = _removeLocaleCode(key)
    if (noLocaleKey === key) return obj[key]
    return obj[`${noLocaleKey}_${currentCode}` as K]
}

export const handleArrayI18n = <A extends object, K extends keyof A>(
    arr: Array<A>,
    keys: K[],
    locale: string,
) => {
    const localizedValues: Array<
        Record<string, string | undefined> & Omit<A, (typeof keys)[number]>
    > = []
    ...
    for (const item of arr) {
        ...
        for (itemKey in item) {
            ...
            if (noLocaleKey && noLocaleKeys.includes(noLocaleKey)) {
                localizedItem.set(
                    noLocaleKey,
                    item[`${noLocaleKey}_${locale}` as K],
                )
                continue
            }
