...
const { t } = useI18n()
const { value: inputValue, errorMessage: error } = useField<File | null | undefined>(props.name)
const { checkFiles } = useFilesCheck({ accept: props.accept, limitSize: props.limitSize })
  ...
  <div class="relative flex flex-col items-start justify-start">
    <div class="flex flex-wrap items-center gap-4">
      <input
        ...
        @change="handleChange"
      />
      <ElButton size="large" :type="error ? 'danger' : 'primary'" plain @click="fileInput?.click()">
        {{ label }}
      </ElButton>
    ...
    <span v-if="error" class="m-1 text-xs text-[var(--el-color-danger)]">{{ t(error) }}</span>
    <ElButton
