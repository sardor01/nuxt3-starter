...
<template>
  <div>
    <ElTable v-loading="!!loading" :data="data" size="large" border>
      ...
      <ElTableColumn v-if="can.edit || can.delete" align="center" width="220" fixed="right" :resizable="false">
        <template #default="scope">
          <ElButton v-if="can.edit" :loading="loading" @click="emit('actions', 'edit', scope.row[rowKey])">
            Edit
          </ElButton>
          <ElButton v-if="can.delete" :loading="loading" type="danger" @click="handleDelete(scope.row[rowKey])">
            Delete
          </ElButton>
        </template>
      </ElTableColumn>
