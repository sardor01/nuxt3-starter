...
const isHexColor = (hexColor: string) => {
  ...
  return (
    typeof hexColor === 'string' && hexColor.startsWith('#') && hex.length === 6 && !Number.isNaN(Number(`0x${hex}`))
  )
}
...
<template>
  <div class="relative flex items-center justify-center overflow-hidden" :class="[bgClass, loaderClass]">
