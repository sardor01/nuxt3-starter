...
const title = computed(() => {
    const title = route.meta.title as
        | string
...
<template>
    <Html :lang="head.htmlAttrs?.lang" class="leading-extra-normal">
        <Head>
            <Title>{{
                title && (title.dynamic ? title.value : t(title.value))
            }}</Title>
        ...
        <Body>
            <div class="relative flex bg-white text-neutral-950">
                <div
                    class="relative flex min-h-screen max-w-full flex-auto flex-col backface-hidden"
                >
                    <TheHeader />
